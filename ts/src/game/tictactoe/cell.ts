...
class Cell extends GridObject {
    ...
    constructor(AIActive: boolean, position?: Vector, cellMinMax?: MinMax, gameObjects?: GameObject[]) {
        super(position);
        if (!AIActive) {
            ...
            for (let x = 0; x < 3; x++) {
                ...
                for (let y = 0; y < 3; y++) {
                    ...
                    let node: Node = new Node(AIActive, nodePos, minMaxNode)
            ...
            for (let x = 0; x < 3; x++) {
                this.nodes.push([]);
                for (let y = 0; y < 3; y++) {
                    let node: Node = new Node(AIActive);
                    this.nodes[x].push(node);
                }
            }
        }
    ...
    setDrawTypeForAI(xIndex: number, yIndex: number, currentActivePlayer: string) {
        ...
        if (this.nodes[xIndex][yIndex].setDrawType(true, currentActivePlayer)) {
            this.checkWinState(true, this.nodes);
                ...
                for (let x = 0; x < 3; x++) {
                    for (let y = 0; y < 3; y++) {
                        if (this.nodes[x][y].nodeMinMax.pointIntersects(mouseClick)) {
                            if (this.nodes[x][y].setDrawType(false, currentActivePlayer)) {
                                this.checkWinState(false, this.nodes);
                                pickedNode.nodeX = x;
                                pickedNode.nodeY = y;
                                pickedNode.picked = true;
                            }
